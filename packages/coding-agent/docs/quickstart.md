...
This page gets you from install to a useful first Supreme Agent session.
...
Both commands fetch versioned Supreme Agent release artifacts and install the `supreme-agent` command. The inherited npm workspace identifiers in the source tree are not the public install path.

Then start Supreme Agent in the project directory you want it to work on:
...
Supreme Agent can use subscription providers through `/login`, or API-key providers through environment variables or its auth file.

### Option 1: Subscription Login

Start Supreme Agent and run:
...
Set an API key before launching Supreme Agent:
...
Once Supreme Agent starts, type a request and press Enter:
...
## Give Supreme Agent Project Instructions

Supreme Agent loads context files at startup. Add an `AGENTS.md` file to tell it how to work in a project:
...
Restart Supreme Agent, or run `/reload`, after changing context files.
...
```bash
supreme-agent @README.md "Summarize this"
supreme-agent @src/app.ts @src/app.test.ts "Review these together"
```
...
```bash
supreme-agent -c                  # Continue the most recent session
supreme-agent -r [path|id]        # Browse sessions or open a specific session
```

Inside Supreme Agent, use `/resume`, `/new`, `/tree`, `/fork`, and `/clone` to manage sessions. Persistent sessions run in worker processes, so closing the TUI detaches from the agent rather than necessarily stopping it. Use `supreme-agent agents` to inspect or reattach to active work.
...
- [Using Supreme Agent](usage.md) - interactive mode, slash commands, sessions, context files, and CLI reference.
- [Providers](providers.md) - authentication and model setup.
- [Settings](settings.md) - global and project configuration.
- [Keybindings](keybindings.md) - shortcuts and customization.
- [Supreme Agent Packages](packages.md) - install shared extensions, skills, prompts, and themes.
