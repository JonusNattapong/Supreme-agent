...
<h1 align="center">Supreme Agent CLI</h1>
...
- [Customization](#customization)
  ...
  - [Themes](#themes)
  - [Supreme Agent Packages](#supreme-agent-packages)
...
Then just talk to Supreme Agent. By default, Supreme Agent gives the model one tool: `ipython`. The model uses the persistent kernel to read files, run commands, edit code, and inspect data. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [Supreme Agent packages](#supreme-agent-packages).
...
For each built-in provider, Supreme Agent maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Supreme Agent can receive the follow-up shortcut.
...
```bash
supreme-agent -c                  # Continue most recent session
supreme-agent -r [path|id]        # Browse past sessions or resume one directly
supreme-agent --no-session        # Ephemeral mode (don't save)
supreme-agent --fork <path|id>    # Fork specific session file or ID into a new session
```
...
Supreme Agent stable builds fetch `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` and remain on the beta channel. Override the base URL with `PRIME_AGENT_DOWNLOAD_BASE_URL`. Disable version checks with `PI_SKIP_VERSION_CHECK=1`.

Use `--offline` or `PI_OFFLINE=1` to disable startup network operations, including update checks and package update checks.

## Context Files

Supreme Agent loads `AGENTS.md` (or `CLAUDE.md`) at startup from:
...
Place in `~/.supreme/agent/prompts/`, `.supreme/agent/prompts/`, or a [Supreme Agent package](#supreme-agent-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).

### Skills

On-demand capability packages following the [Agent Skills standard](https://agentskills.io). At startup, Supreme Agent gives the model each visible skill's name, type, description, and location. The full `SKILL.md` stays out of context until the model inspects it with `ipython` or you explicitly invoke `/skill:name`.
...
Skills can also be Python-backed. A Python skill is a normal skill directory with `SKILL.md` plus a Python package at `src/<import_name>/`. Supreme Agent installs it into the persistent Python kernel and exposes it by import name, so the model can call it directly, inspect it with `help()`, or use any console scripts the skill declares.

Place in `~/.supreme/agent/skills/`, `~/.agents/skills/`, `.supreme/agent/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [Supreme Agent package](#supreme-agent-packages) to share with others. See [docs/skills.md](docs/skills.md).

Supreme Agent ships with a built-in `websearch` skill (Google search via the [Serper](https://serper.dev) API). It loads by default; run `/login`, switch to **MCP Connections**, and choose "Serper (web search)" to add your key. Disable it with `bundledSkills.websearch: false`, or override it with your own `websearch` skill in any location above. See [docs/skills.md#built-in-skills](docs/skills.md#built-in-skills).
...
TypeScript modules that extend Supreme Agent with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. Supreme Agent waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make Supreme Agent look like Claude Code
- Games while waiting (yes, Doom runs)
- ...anything you can dream up

Place in `~/.supreme/agent/extensions/`, `.supreme/agent/extensions/`, or a [Supreme Agent package](#supreme-agent-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and Supreme Agent immediately applies changes.

Place in `~/.supreme/agent/themes/`, `.supreme/agent/themes/`, or a [Supreme Agent package](#supreme-agent-packages) to share with others. See [docs/themes.md](docs/themes.md).
...
```bash
supreme-agent package install npm:@foo/prime-agent-tools
supreme-agent package install npm:@foo/prime-agent-tools@1.2.3  # pinned version
supreme-agent package install git:github.com/user/repo
supreme-agent package install git:github.com/user/repo@v1       # tag or commit
supreme-agent package install git:git@github.com:user/repo
supreme-agent package install https://github.com/user/repo
supreme-agent package install ssh://git@github.com/user/repo
supreme-agent package remove npm:@foo/prime-agent-tools
supreme-agent package list
supreme-agent package update                                  # update packages, except pinned versions
supreme-agent package update npm:@foo/prime-agent-tools       # update one package
supreme-agent update                                          # update Supreme Agent
supreme-agent update --force                                  # reinstall Supreme Agent even if current
supreme-agent config                                          # enable/disable package resources
```
...
Without a `pi` manifest, Supreme Agent auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
The package architecture, extension model, and source package names still reflect that upstream lineage while the distributed command and release artifacts are branded for Supreme Agent.
...
Run `supreme-agent help` for the command list and `supreme-agent help <command>` for details.

### Agent Commands

```bash
supreme-agent agents                         # Search running, idle, and inactive sessions
supreme-agent list [--all]                   # List active or saved agents
supreme-agent attach <agent>                 # Attach the interactive UI
supreme-agent stop <agent>                   # Stop one agent
supreme-agent rename <agent> <name>          # Rename an agent
supreme-agent send <agent> <message>         # Send an agent-to-agent message
supreme-agent status                         # Show background service status
supreme-agent doctor [--fix]                 # Inspect or safely clean up background services
...
```bash
supreme-agent schedule list [--all] [agent]
supreme-agent schedule add <agent> <schedule> -- <message>
supreme-agent schedule cancel <job-id>
```
...
```bash
supreme-agent package install <source> [--local]
supreme-agent package remove <source> [--local]
supreme-agent package list
supreme-agent package update [source]
supreme-agent update [--force]                   # Update Supreme Agent itself
supreme-agent config                             # Enable/disable package resources
```
...
In print mode, Supreme Agent also reads piped stdin and merges it into the initial prompt:

```bash
cat README.md | supreme-agent -p "Summarize this text"
```
...
Use `supreme-agent model list [search]` to list available models.
...
Use `supreme-agent session export <file> [output]` to export a saved session to HTML.
...
```bash
supreme-agent @prompt.md "Answer this"
supreme-agent -p @screenshot.png "What's in this image?"
supreme-agent @code.ts @test.ts "Review these files"
```
...
supreme-agent "List all .ts files in src/"

# Non-interactive
supreme-agent -p "Summarize this codebase"

# Non-interactive with piped stdin
cat README.md | supreme-agent -p "Summarize this text"

# Different model
supreme-agent --provider openai --model gpt-4o "Help me refactor"

# Model with provider prefix (no --provider needed)
supreme-agent --model openai/gpt-4o "Help me refactor"

# Model with thinking level shorthand
supreme-agent --model sonnet:high "Solve this complex problem"

# Limit model cycling
supreme-agent --models "claude-*,gpt-4o"

# Restrict to the built-in Python REPL tool
supreme-agent --tools ipython -p "Review the code"

# High thinking level
supreme-agent --thinking high "Solve this complex problem"
```
...
| `PI_SKIP_VERSION_CHECK` | Skip the Supreme Agent version update check at startup. This prevents the release manifest request |
...
| `PRIME_AGENT_DOWNLOAD_BASE_URL` | Override the Supreme Agent release manifest and tarball base URL |
...
| `PRIME_AGENT_TRACES_BASE_URL` | Override the Supreme Agent trace upload API base URL |
...
- [Supreme Agent AI](../ai): Core LLM toolkit
- [Supreme Agent Core](../agent): Agent framework
- [Supreme Agent TUI](../tui): Terminal UI components