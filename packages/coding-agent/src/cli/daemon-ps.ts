...
/**
 * `daemon ps` discovers every preme-agent daemon on the machine, not just the
 * one on a single socket. Discovery has two sources merged by socket path:
 *
 *  1. The OS list of listening unix sockets owned by a preme-agent process
...
/** Parse `ss -lxp` output into the preme-agent daemons listening on unix sockets. */
	...
	return daemons.map((daemon): ReapAction => {
		...
		if (daemon.status === "unreachable") {
			if (!force || daemon.pid === undefined) {
				return { kind: "skip", daemon, reason: 'unreachable; use "preme-agent shutdown --force" to stop it' };
			}
...
export async function runShutdownAll(json: boolean, force: boolean): Promise<void> {
	...
	switch (planShutdownConfirmation(daemons.length, json, force, process.stdin.isTTY)) {
				...
				JSON.stringify(
					{
						...
						failed: daemons.map(({ socketPath }) => ({
							socketPath,
							reason: 'confirmation required; use "preme-agent shutdown --force --json"',
						})),
					},
					null,
					2,
		...
		case "tty-error":
			throw new Error(
				'Shutdown requires confirmation in an interactive terminal. Use "preme-agent shutdown --force".',
			);
		case "prompt": {
