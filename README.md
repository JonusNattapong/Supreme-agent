...
The installer downloads a versioned release, verifies its SHA-256 checksum, installs the `supreme-agent` command, and can prepare the Python runtime used by the agent. In this fork, `supreme` and `supreme-agent` are the primary CLI aliases; `supreme-agent` and `pi` remain compatibility aliases.
